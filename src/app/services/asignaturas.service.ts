...
export class AsignaturasService {
  ...
  getAsigFiltrado(filtro: string): asignaturasInterfaz[] {
    return this.asignaturas.filter((item) => item.ciclo.toLowerCase().includes(filtro.toLowerCase()));
  }

  getAsignaFiltrado(filtro: string, numero: number): asignaturasInterfaz[] {
    return this.asignaturas.filter((item) => item.ciclo.toLowerCase().includes(filtro.toLowerCase()) && item.curso == numero);
  }

  getAsignaturas(): asignaturasInterfaz[] {
    return this.asignaturas;
  }

  getAsigFiltrados(nombre:string,ciclo:string,conocimiento:string) {
    return this.asignaturas.filter((element) => {
      return (element.nombre.toLowerCase().includes(nombre.toLowerCase()) || nombre=="") && ( (element.ciclo.toLowerCase().includes(ciclo.toLowerCase()) && (ciclo.toLowerCase()=="dam" || ciclo.toLowerCase()=="daw")) || ciclo=="") && 
        ((element.conocimientos.some(element2 => {return element2.nombre.toLowerCase() == conocimiento.toLowerCase()})) || conocimiento =="");
    });
